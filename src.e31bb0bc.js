...
adresseInput.oninput = function (params) {
  ...
  if (adresseTekst.length > 6) {
    (0, _api_service.searchAddress)(adresseTekst).then(function (result) {
      ...
      menu.innerHTML = "";
      result.data.adresser.forEach(function (item, index) {
        menu.appendChild(createMenuItem(item.adressetekst + ", " + item.postnummer + " - " + item.poststed));
      });
    });
  }
};
...
myLink.onclick = function () {
  (0, _api_service.searchAddress)(adresseTekst).then(function (res) {
    ...
    if (data) {
      (0, _add_waypoint.addWaypoint)(data.representasjonspunkt.lon, data.representasjonspunkt.lat, "Opps\xF8kt hus: ".concat(data.adressetekst, ", ").concat(data.postnummer, " - ").concat(data.poststed), projectTo, vectorLayer);
